...
bot.once("spawn", function () {
  let timer = setInterval(function() {
    let nearby_players = Object.values(bot.entities).filter(e => e.type === 'player' && bot.entity.position.distanceTo(e.position) < disconnect_radius).map(e => e.username); // [Username, Username]
